...
async function getCourse(cid: string): Promise<Course> {
  ...
  try {
    ...
    const response = await fetch(`http://nginx/course/${cid}`, {
      headers: {
...
async function getLesson(lid: string): Promise<Lesson | null> {
  ...
  try {
    ...
    const response = await fetch(`http://nginx/lesson/${lid}`, {
      headers: {
        'Content-Type': 'application/json',
      },
      cache: 'no-store',
              ...
              <ScrollArea className="h-[calc(100vh-20rem)]">
                <div className="p-4">
                  {course.modules?.map((module, index) => (
                    ...
                    >
                      <div className={`flex items-center p-3 mb-2 rounded-md transition-colors ${
                        module.lid.toString() === lid ? 
